...
{
  char* token;
  if (!command) return SIM_CMDUNKNOWN;
  token = strsep(&command, "\t\n\v ");
  *remainder = command;
  if (streq(token, "quit")) return SIM_CMDQUIT;
  if (streq(token, "q")) return SIM_CMDQUIT;
  if (streq(token, "exit")) return SIM_CMDQUIT;
  if (streq(token, "help")) return SIM_CMDHELP;
  if (streq(token, "h")) return SIM_CMDHELP;
  if (streq(token, "?")) return SIM_CMDHELP;
  if (streq(token, "load")) return SIM_CMDLOAD;
  if (streq(token, "save")) return SIM_CMDSAVE;
  if (streq(token, "step")) return SIM_CMDSTEP;
  if (streq(token, "s")) return SIM_CMDSTEP;
  if (streq(token, "run")) return SIM_CMDRUN;
  if (streq(token, "r")) return SIM_CMDRUN;
  if (streq(token, "inspect")) return SIM_CMDINSPECT;
  if (streq(token, "i")) return SIM_CMDINSPECT;
  if (streq(token, "disassemble")) return SIM_CMDDISASSEMBLE;
  if (streq(token, "dasm")) return SIM_CMDDISASSEMBLE;
  if (streq(token, "dump")) return SIM_CMDDUMP;
  if (streq(token, "d")) return SIM_CMDDUMP;
  if (streq(token, "break")) return SIM_CMDBREAK;
  if (streq(token, "unbreak")) return SIM_CMDUNBREAK;
  if (streq(token, "symbols")) return SIM_CMDSYMBOLS;
  if (streq(token, "set-slots")) return SIM_CMDSETSLOTS;
  if (streq(token, "tree")) return SIM_CMDTREE;
  if (streq(token, "trace")) return SIM_CMDTRACE;
  if (streq(token, "untrace")) return SIM_CMDUNTRACE;
  if( streq(token, "reset")) return SIM_CMDRESET;
  ...
  while (!quit && (in = readline("DLX>"))) {
    ...
    if (command_code = sim_command(in, &out)) {
      ...
      switch (command_code) {
        ...
        case SIM_CMDLOAD:
          if( asm_result ) asm_destroy_asm_info( asm_result );
        ...
        case SIM_CMDSETSLOTS:
          ...
          break;
        case SIM_CMDTRACE:   vm_trace(); break;
        case SIM_CMDUNTRACE: vm_untrace(); break;
        case SIM_CMDRESET:   vm_reset(); break;
      }
    }
    else {
      free(histbuff);
