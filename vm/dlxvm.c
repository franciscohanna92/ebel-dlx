...
void vm_set_branch_delay_slots( int nb ) {
  ...
  else dlx_vm.branch_delay_slots = nb;
}
...
int vm_internal_breakpoint_value( const char* label ) {
  ...
  if( i < 0 ) printf("Invalid label, must greater than 0\n");
  if( i > dlx_vm.code_length ) printf("Invalid label, out of code segment.\n");
...
void vm_internal_decode( dlx_instr_decode *instr ) {
  ...
  instr->ent     = ((instr->opcode==0)?DLX_INSTR_SP_TABLE + instr->fAlu:
                    ((instr->opcode==1)?DLX_INSTR_FP_TABLE + instr->fFpu:
                     (DLX_INSTR_TABLE + instr->opcode)));
...
int vm_internal_execute( dlx_instr_decode *instr ) {
  ...
  case FPARITH: 
    ...
    case EQF:
      dlx_vm.registers.FPSR = 
        ( (DLX_FLOAT)dlx_vm.registers.F[instr->r1] == 
          (DLX_FLOAT)dlx_vm.registers.F[instr->r2] )?1:0;
      break;
    ...
    case EQD:
      dlx_vm.registers.FPSR = 
        ( *((DLX_DOUBLE*)(dlx_vm.registers.F+instr->r1)) == 
          *((DLX_DOUBLE*)(dlx_vm.registers.F+instr->r1)) )?1:0;
      break;
    ...
    case SEQU:
      dlx_vm.registers.R[ instr->rd ] = 
        (DLX_WORD)((DLX_UWORD)dlx_vm.registers.R[ instr->r1 ] == 
                   (DLX_UWORD)dlx_vm.registers.R[ instr->r2 ])?1:0;
      break;
    ...
    case SEQ:
      dlx_vm.registers.R[ instr->rd ] = 
        ((DLX_WORD)dlx_vm.registers.R[ instr->r1 ] == 
         (DLX_WORD)dlx_vm.registers.R[ instr->r2 ] )?1:0;
      break;
...
int vm_disassemble( DLX_UWORD start, DLX_WORD length ) {
  ...
  for( i = 0; i<length || loop < 0; i++ ) {
    if( start + i*4 >= dlx_vm.code_length ) return 1;
    //-- Fetch --//
