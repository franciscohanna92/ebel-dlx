...
void asm_gencode_build_memory( Node *node ) {
  ...
  while( node != NULL ) {
    ...
    case A_CMDDATA: 
      ...
      while( n != NULL ) {
        if( n->type == A_VALUE ) n->fvalue = n->value;
        if( n->type == A_FVALUE ) n->value = n->fvalue;

        switch( asm_data_type_fixnum( node->param[1]->identifier ) ) {
        case 1: STORE_BYTE_TO_BYTE( ptr, n->value ); break;
        case 2: STORE_SHORT_TO_BYTE( ptr, n->value ); break;
        case 4: STORE_INT_TO_BYTE( ptr, n->value ); break;
        case 8: STORE_LONG_TO_BYTE( ptr, n->value ); break;
        }
        switch( asm_data_type_flonum( node->param[1]->identifier ) ) {
        case 4: STORE_FLOAT_TO_BYTE( ptr, n->fvalue ); break;
        case 8: STORE_DOUBLE_TO_BYTE( ptr, n->fvalue ); break;
        }
        ptr += data_length;
        n = n->next;
      }
